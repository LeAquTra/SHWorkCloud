...
- **若你的库是更早版本建的，要按顺序补增量脚本**（都是幂等的，可重复执行）：

  ```bash
  # v2.0 → v2.1：个性属性字段（昵称/签名/性别/生日）
  sudo mysql shwork_cloud < init_sql/migration_v2.1.sql
  # v2.1 → v2.2：公告表 announcement + 头像冷却时间戳 sys_user.avatar_updated_at
  sudo mysql shwork_cloud < init_sql/migration_v2.2.sql
  ```

  > ⚠️ **不做 v2.2 会直接报错**：`updateAvatar` 现在会写 `sys_user.avatar_updated_at`，
  > 列不存在时用户换头像会 SQL 失败（`Unknown column 'avatar_updated_at'`）。
  > 公告相关接口如果发现表不存在，也会整片 500。升级前先备份（§9.3）。
...
| 2 | **RAM 子用户** | 授权见下方**路线 A（系统策略，省事）**或**路线 B（自定义策略，精确）**；生成 **AK/SK** → 填 `STS_AK`/`STS_SK`。⚠️ 不给权限的话，上传会「前面全正常、就是传不上去」 |
| 3 | **STS 角色** | 建一个允许该 RAM 用户 `AssumeRole` 的角色，角色 ARN → 填 `STS_ROLE_ARN` |
| 4 | **跨域 CORS** | ⚠️ **必须配**，否则浏览器直传会被浏览器拦下（见下） |

**路线 A（省事）：只用系统策略**

不想维护自定义策略的话，给 RAM 用户加这两条系统策略就够：

| 系统策略 | 何时需要 | 作用 |
|----------|---------|------|
| **`AliyunOSSFullAccess`** | **必加** | 管理 OSS。本项目的直传、分片、下载、删除、对账全在里面 |
| `AliyunSTSAssumeRoleAccess` | 仅用 `/oss/sts` 可选路径时 | 允许调用 `AssumeRole` 换取临时凭证 |

> ⚠️ **OSS 的产品系统策略只有两个**：`AliyunOSSFullAccess` 与 `AliyunOSSReadOnlyAccess`
> （[官方清单](https://www.alibabacloud.com/help/zh/oss/user-guide/oss)）。
> **没有 `AliyunOSSReadWriteAccess` 这种中间档** —— 只读的传不了文件，所以只能选 FullAccess。
>
> ⚠️ 那份清单里还有 `AliyunOSSBatchOperationJobRolePolicy`、`AliyunOSSLoggingDefaultRolePolicy`、
> `AliyunServiceRolePolicyForOssMfd` 等，它们是**服务角色/服务关联角色专用**，
> 官方明确写了"**请勿将本策略授权给服务角色之外的 RAM 身份**"，别被名字里的 OSS 骗了。
>
> **代价**：`AliyunOSSFullAccess` 给的是**账号下所有 Bucket 的完整管理权**（含删桶、改 ACL），
> 不只是你用的那一个；`AliyunSTSAssumeRoleAccess` 允许扮演**任意角色**。
> 非商用单桶项目完全够用；若该账号下还有别的桶，建议改用下面的路线 B。
> 另外系统策略由阿里云维护、内容可能变化，且**不能修改**。
>
> 用到 STS 的话，**RAM 角色也给它 `AliyunOSSFullAccess`** 即可 ——
> 因为 `StsService` 在 AssumeRole 时会下发一个收窄的会话策略
> （限到 `homework/{userId}/*`，且只有 Put/ListParts/Abort），
> 临时凭证最终交给浏览器的权限是**被代码夹紧过的**。

**路线 B（精确）：自定义策略**

后端用这对 AK/SK **直接构造 OSS 客户端**（`OssClientConfig`），所以 RAM 用户必须有下面这些动作。
自定义策略 JSON（把 `sakura-4826` 换成你的桶名）：
...
| Action | 代码里谁在用 | 少了会怎样 |
|--------|-------------|-----------|
| `oss:GetObject` | 下载/在线预览/正文提取、`commit` 时 `sizeOf()`、GET 预签名地址 | **上传能传上去但 commit 失败**（很多"传完了却报错"就是这个） |
| `oss:PutObject` | 浏览器直传（PUT 预签名）、分片上传、换头像、题库图片 | **上传直接失败**；换头像也失败 |
| `oss:DeleteObject` | 删文件、换头像删旧对象、对账清理 | 索引删了但 OSS 留垃圾，持续计费 |
| `oss:AbortMultipartUpload` | 放弃上传 | 未完成分片残留、持续计费 |
| `oss:ListParts` | 断点续传校对上传播 | 续传失效，只能重传 |
| `oss:ListObjects` | 每日 OSS 对账、运维扫描 | 对账任务报错 |

注意：`oss:ListObjects` 的作用域是**桶本身**（`...:sakura-4826`，**不带** `/*`），对象级动作才是 `.../*`；
另外 **`HeadObject` 归 `oss:GetObject`**，阿里云没有单独的 Head 权限。
若以后日志里出现 `GetBucketInfo`/`GetBucketLocation` 类的 `AccessDenied`，再补 `oss:GetBucketInfo` 即可（当前代码不调它）。

**STS 角色（只有用可选的 `/oss/sts` 路径才需要）**

前端走预签名 URL，`/oss/sts` 是可选路径。但 `application-prod.yaml` 里 `STS_ROLE_ARN` 是
**没有默认值**的占位符，**不填值应用起不来**（哪怕不用它）。要用该路径则配三样：

```json
// ① 角色的权限策略：会话策略只能收窄不能放大，所以角色本身必须先允许这些动作
{"Version":"1","Statement":[{"Effect":"Allow",
 "Action":["oss:PutObject","oss:ListParts","oss:AbortMultipartUpload"],
 "Resource":["acs:oss:*:*:sakura-4826/homework/*"]}]}

// ② 角色的信任策略：允许该 RAM 用户扮演
{"Version":"1","Statement":[{"Action":"sts:AssumeRole","Effect":"Allow",
 "Principal":{"RAM":["acs:ram::<账号ID>:user/<RAM用户名>"]}}]}

// ③ RAM 用户再加一条：允许它 AssumeRole 这个角色
{"Version":"1","Statement":[{"Effect":"Allow","Action":"sts:AssumeRole",
 "Resource":["acs:ram::<账号ID>:role/<角色名>"]}]}
```

> `<账号ID>` 不用另找 —— 你现有 `STS_ROLE_ARN`（`acs:ram::1234567890:role/xxx`）中间那串数字就是。
> 角色「最大会话时长」需 ≥ `aliyun.sts.duration-seconds`（默认 1800）。

**CORS 规则（浏览器直传必需）**

阿里云控制台路径：**对象存储 OSS → Bucket 列表 → 点进你的 Bucket → 左侧「数据安全」→「跨域设置」→ 创建规则**。
一张规则表就是下面 5 个字段：

| 控制台字段 | 填什么 | 说明 |
|-----------|--------|------|
| **来源 (Origin)** | 先填 `*`；稳定后换成具体来源（见下） | 必须与浏览器地址栏**逐字符一致**（含协议与端口） |
| **允许 Methods** | 勾 `GET` `PUT` `POST` `HEAD` | `PUT` 是上传，`HEAD` 给断点续传探测 |
| **允许 Headers** | `*` | 至少要覆盖 `content-type` |
| **暴露 Headers (Expose-Headers)** | `ETag` | ⚠️ **漏了它，大文件分片上传最后一步必失败** |
| **缓存时间（秒）** | `600` | 预检结果缓存时长，改配置后最多 10 分钟才生效 |

> **强烈建议第一次直接填 `*`。** 直传 OSS 的请求**不携带任何 Cookie/凭据**
> （签名信息都在 URL 里，AK/SK 从不进浏览器），所以 `*` 是安全的；
> 而它能一次性覆盖 `https`/`http`、apex/`www`、以及联调期的 `http://localhost:5173`，
> **直接绕开"来源写错"这个最常见的坑**。想收敛时再换成具体来源。

**要收敛时，「来源」这么填**（`www` 与 apex、`https` 与 `http` 是**四个不同的 Origin**）：

```
https://solohelper.xyz
https://www.solohelper.xyz
```

> - 若你按 §6.2 ④ 做了 `apex → www` 的 301 跳转，浏览器地址栏最终只剩一个域名，
>   那**这里只需要列那一个**；没做跳转就必须把两个都列上。
> - 联调期如果前端还跑在 `npm run dev`（`http://localhost:5173`），也要列上它，
>   或者干脆用 `*`。
> - `Origin` **不包含路径**，填 `https://solohelper.xyz/` （带斜杠）是**错的**。

配完**立即生效**，不需要重启任何服务。若浏览器仍走旧的预检结果，强刷（`Ctrl+F5`）
或等「缓存时间」到期即可。

#### ⚠️ 症状对照：上传失败几乎都是这一步没做

**「登录/注册/建文件夹都正常，只有上传失败」= 极大概率就是 CORS 没配。**

原因：登录、注册、建文件夹走的都是你自己域名的同源 `/api`，**不涉及跨域**；
而 **浏览器直传 OSS 是跨域请求**。并且 `PUT` **不属于 CORS 简单方法**
（只有 `GET`/`HEAD`/`POST` 是），所以浏览器**一定**会先发 `OPTIONS` 预检；
桶上没有匹配的跨域规则时预检就被拦，前端只能看到一个笼统的
"网络错误，上传中断"（XHR 的 `onerror`），**看不到任何 HTTP 状态码** —— 这一步最容易误判成后端 bug。

> ⚠️ **但"上传失败"有两个独立原因：CORS 没配 + RAM 没权限。两个都得修。**
> 好消息是上传链路的**前三个接口根本不碰 OSS**：`instantUpload` 对新文件只查数据库、
> `/oss/ticket` 只写 Redis/DB、`/oss/put-url` 是**本地算签名**。
> 所以"前面全正常、就是传不上去"与"两道都没配"完全吻合。

**诊断〇（最快，先做这个）：去个人资料页换一次头像。**

头像走的是**服务端 `PutObject`**（`AvatarService.upload` → `ossSignService.put`），
**完全不经过浏览器跨域**，所以它能干净地把两个故障分开：

| 换头像结果 | 结论 |
|-----------|------|
| ✅ 成功 | AK/SK、桶名、endpoint、`oss:PutObject` 全正常 → 文件上传失败**纯粹是 CORS** |
| ❌ 失败 | 是权限/桶的问题 → 先按上面的 RAM 策略修，此时上传失败是**两个原因叠加** |

**诊断一：模拟浏览器的预检请求**（Origin 换成你**实际**访问前端的地址，协议/端口都要一致）

```bash
# 两个域名各跑一次（把 Bucket 与 endpoint 换成你自己的）
for O in https://solohelper.xyz https://www.solohelper.xyz; do
  echo "===== Origin: $O"
  curl -s -i -X OPTIONS 'https://sakura-4826.oss-cn-beijing.aliyuncs.com/homework/probe.txt' \
    -H "Origin: $O" \
...
| 返回 | 含义 |
|------|------|
| `403` 且**完全没有** `Access-Control-Allow-*` 响应头 | 没配 CORS（或来源不匹配）→ 按上表补配 |
| `200`/`204` + `Access-Control-Allow-Origin: <你的来源>`（或 `*`）+ `Allow-Methods` 含 `PUT` | CORS 正常，去查别的原因 |

> 这条命令**不需要任何密钥**，因为是匿名预检 —— 会不会被放行只取决于 CORS 规则，
> 正好用来单独验证这一项。两个域名都过了再回浏览器试上传。

**诊断二：确认不是签名/地域问题**（这条绕过 CORS，从服务器侧发）

```bash
# 先用 /oss/ticket + /oss/put-url 拿到 url 与 contentType，再：
curl -i -X PUT '<put-url 返回的 url>' \
  -H 'Content-Type: <put-url 返回的 contentType>' \
  --data-binary @test.txt
```

- 这条**成功**、浏览器却失败 → **100% 是 CORS，代码无责**；
- 这条也失败（如 `SignatureDoesNotMatch`）→ 才是签名或配置问题，见 §9.5。

**最快的定位方式**：浏览器 `F12` → **Console**，那条 CORS 报错会**明确打印它认为的 Origin**，
拿它和你在 OSS 里填的「来源」逐字符比对即可。联调期最常见的错就是
**CORS 里配了正式域名，但实际从 `http://localhost:5173` 访问**（联调期可先把来源填 `*`，稳定后再收敛）。

**其他两个常见失败点**

- **漏配 `Expose-Headers: ETag`**：分片上传会卡在最后一步 `complete`（前端现在会直接提示你补这一项）。
- **来源写了 `*` 但带了凭据**：本项目直传不带 Cookie，所以 `*` 可用；若你后来改用带凭据的请求，`*` 会失效，必须写具体域名。
...
| `OSS_ENDPOINT` | `oss-cn-beijing.aliyuncs.com` | 与 Bucket 地域一致。**协议由代码强制为 HTTPS**（`OssClientConfig`），所以这里写裸域名或带 `https://` 都可以；⚠️ 千万别指望 SDK 默认走 https —— 它默认是 **http**，会导致 https 页面拦掉上传（§9.5） |
...
# ⚠️ 这里必须是**角色** ARN（`role/`），不是 RAM 用户的 ARN（`user/`）！
#    写成 acs:ram::123:user/xxx 的话，/oss/sts 调 AssumeRole 会直接报参数错误。
#    只用预签名 URL 直传（前端默认路径）可以不建角色，但这一项**必须非空**，否则 prod 启动失败。
...
**② 人机验证：`app.captcha.*`（登录 / 注册 / 上传）**

后台的图片验证码题库会在**登录、注册发送邮件码、上传文件**三处做人工验证。开关都可从环境变量覆盖：

| 环境变量 | 默认 | 作用 |
|------|:---:|------|
| `CAPTCHA_ENABLED` | `true` | **总开关**。出事时（题库被误删、验证码图片在 OSS 侧访问不了）设为 `false` 并重启即可全关，**不必重新打包** |
| `CAPTCHA_REQUIRE_ON_LOGIN` | `true` | 登录前要人机验证 |
| `CAPTCHA_REQUIRE_ON_REGISTER` | `true` | 注册发邮件码之前要人机验证 |
| `CAPTCHA_REQUIRE_ON_UPLOAD` | `true` | 上传文件（申请凭证）之前要人机验证 |
| `CAPTCHA_UPLOAD_PASS_MINUTES` | `10` | 通过一次后的免验证窗口（分钟）：整文件夹上传只打扰一次 |

```bash
# /etc/shworkcloud/env 追加即可（不配也有默认值）
CAPTCHA_ENABLED=true
CAPTCHA_REQUIRE_ON_LOGIN=true
CAPTCHA_REQUIRE_ON_REGISTER=true
CAPTCHA_REQUIRE_ON_UPLOAD=true
```

> ⚠️ **三条必须知道的行为**（否则容易把正常行为误判成故障）：
> 1. **题库为空时不会真的要求验证**。全新部署、或题目被删光时，服务端自动降级为"不要求" ——
>    否则所有人都会被「请先完成人机验证」挡在门外，而用户根本没法完成验证。
>    也就是说：**上传题目后验证码才真正生效**，在此之前登录/上传一切照旧；
> 2. **改开关不用重新打包**：改 `/etc/shworkcloud/env` → `systemctl restart shworkcloud`；
> 3. **冒烟脚本会被人机验证挡住**（登录与申请上传凭证返回 `40105`），
>    处理方式见 §9.5 与脚本的 `-CaptchaPassToken` 参数。

**③ `app.register.*`（自助注册）**

默认 `enabled: true` —— 用户可自行注册登录。注册环节要不要图片验证码，现在由
`CAPTCHA_REQUIRE_ON_REGISTER` 统一控制（原先的 `app.register.require-image-captcha` 已合并过去）。
...
> ⚠️ **第 6.1 节的缓存规则不是可选项，请一定配上。**
> 配错了会出现"发版后部分菜单点不开"的诡异故障，原因见 §9.5 的
...
server {
    listen 80;
    # ⚠️ 有几个域名就写几个（apex 与 www 都写），否则访问没列出的那个会落到默认站点上
    server_name solohelper.xyz www.solohelper.xyz;
...
在 §6 那个 `server { }`（443 段）里**加下面这些 location**：
...
```nginx
    ...
    # ⚠️⚠️ index.html 必须"永远不缓存"。这段不可省，理由见下。
    #   index.html 是构建产物里**唯一不带内容 hash** 的文件，它里面写着
    #   本次构建的 JS/CSS 文件名。浏览器一旦缓存了旧 index.html，
    #   就会继续加载旧主包；而旧主包用动态 import 拉的那些页面 chunk
    #   名字（带 hash）在新构建里已经变了、服务器上已不存在 →
    #   点菜单报 "Failed to fetch dynamically imported module"，
    #   而且**只有你以前没打开过的页面会中招**（打开过的 chunk 还在缓存里），
    #   看起来就像"部分菜单坏了"，极难定位。详见 §9.5。
    location = /index.html {
        root /var/www/shworkcloud;
    ...
    location / {
        # SPA 路由回退：/files/123 这类前端路由要回 index.html，否则刷新会 404。
        # 回退目标 /index.html 会命中上面那条精确匹配 location，所以同样不缓存。
        try_files $uri $uri/ /index.html;
    }

    # 带内容 hash 的构建产物：可以长期强缓存（文件名变了就等于换了文件）。
    # ⚠️ 刻意**不加** try_files 回退到 index.html：文件不存在就老实返回 404。
    #    若回退成 index.html，浏览器会把 HTML 当 JS 模块解析，
    #    报一个毫无线索的 "Failed to fetch dynamically imported module"。
    location ^~ /assets/ {
        root /var/www/shworkcloud;
    ...
    # ⚠️ location /api/ 必须仍然存在
```

> **为什么 `no-store` 而不是 `no-cache`**：`no-cache` 仍会把文件写进磁盘缓存、每次再校验；
> `no-store` 干脆不存。index.html 一次页面加载只取一次（SPA 内部跳转不会重新请求），
> 代价可以忽略，换来的是"发版后绝不会有人卡在旧版本"。

**发版时的正确姿势**（避免新旧文件混在一起）：

```bash
# 先清空旧产物再放新的，避免残留上一版的 chunk
sudo rm -rf /var/www/shworkcloud/assets
sudo cp -r dist/* /var/www/shworkcloud/
sudo chown -R root:root /var/www/shworkcloud
```

> - **`dist/` 根目录下还有 `favicon.svg`**（站点图标，前端 `public/` 目录的产物），
>   `cp -r dist/*` 会一起带过去，`location /` 的 `try_files $uri` 就能命中它 ——
>   不需要额外配 location 或 MIME（Nginx 自带 `mime.types` 里有 `image/svg+xml`）。
>   若只手动拷了 `assets/` 和 `index.html`，浏览器标签页就会退回默认空白图标。
> - 部署完让用户按一次 **`Ctrl+F5`**：`index.html` 已经是 `no-store`，
>   但用户当前页面上那份旧主包还在内存里。
...
### 6.2 两个域名（apex + www）的完整做法

以 `solohelper.xyz` 与 `www.solohelper.xyz` 为例。

**① DNS：两个都要解析到本机公网 IP**

```bash
dig +short solohelper.xyz
dig +short www.solohelper.xyz
# 两条都应返回你的服务器公网 IP；有任一条为空，certbot 就会失败
```

**② Nginx：每个 server 块的 `server_name` 都要写两个**

```nginx
server_name solohelper.xyz www.solohelper.xyz;
```

> 只写一个的话，访问另一个会落到默认站点（看到 Nginx 欢迎页或别的站），
> 而且 `certbot` 申请证书时域名校验也会失败。

**③ 证书：一次给两个域名签同一张证书**

```bash
# 80 站点跑通后（§6 第 1 步），一次性带上两个 -d
sudo certbot --nginx -d solohelper.xyz -d www.solohelper.xyz

# 确认证书覆盖了哪些域名
sudo certbot certificates
# 期望看到 Domains: solohelper.xyz www.solohelper.xyz
```

> 只签了一个域名的话，访问另一个会报证书不受信（`NET::ERR_CERT_COMMON_NAME_INVALID`）。
> 补签：把上面那条命令**两个 `-d` 都带上**再跑一次，certbot 会扩展已有证书。

**④ 选一个主域名，另一个 301 过去（推荐，但可选）**

推荐把 `solohelper.xyz` 跳到 `www.solohelper.xyz`（或反过来，随你）。
这样做的好处是**浏览器地址栏最终只有一个域名**，于是
**OSS 的跨域规则只需要放行那一个来源**，少一个出错面：

```nginx
# 单独一个 server 块做跳转
server {
    listen 443 ssl;
    server_name solohelper.xyz;
    ssl_certificate     /etc/letsencrypt/live/solohelper.xyz/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/solohelper.xyz/privkey.pem;
    return 301 https://www.solohelper.xyz$request_uri;
}

server {
    listen 443 ssl;
    server_name www.solohelper.xyz;
    # ... 正常的 location / 与 location ^~ /api/ ...
}
```

> **也可以不做跳转**，两个域名各跑一份 —— 那就必须在 §2.5 的 OSS 跨域规则里
> **把两个来源都列上**。两种做法都行，别只配一个来源却用另一个域名访问。

**⑤ 改完必做**

```bash
sudo nginx -t && sudo systemctl reload nginx.service
# 两个域名都要能出接口（不是 Nginx 欢迎页）
curl -s -H 'Host: solohelper.xyz'     http://127.0.0.1/api/auth/register-config
curl -s -H 'Host: www.solohelper.xyz' http://127.0.0.1/api/auth/register-config
```
...
# 1.5 跑增量 SQL（有结构变更时必须做，脚本幂等）
#     顺序：v2.1（个性属性）→ v2.2（公告表 + 头像冷却时间戳）
sudo mysql shwork_cloud < init_sql/migration_v2.1.sql
sudo mysql shwork_cloud < init_sql/migration_v2.2.sql
...
# 4. 跑一遍 §8 验收清单
#    换头像 / 公告这两块要专门点一遍：它们依赖 v2.2 新增的表与列

# 5. 前端也要一起发（只要有改动），见 §6.1
#    sudo rm -rf /var/www/shworkcloud/assets && sudo cp -r dist/* /var/www/shworkcloud/
#    ⚠️ 只发 jar 不发 dist，会出现"接口有了但界面没有入口"；反之亦然。
#    发完让用户 Ctrl+F5。
...
| **"网络错误，上传中断" / 上传一点就断（登录注册建文件夹都正常）** | 🔴 **OSS 桶没配跨域 CORS**。`PUT` 不是 CORS 简单方法，浏览器必然先发 `OPTIONS` 预检；桶上无匹配规则 → 预检被拦 → XHR 只报 `onerror`，看不到状态码 | §2.5：用 `curl -X OPTIONS` 模拟预检（带 `Origin` / `Access-Control-Request-Method: PUT`）验证；控制台按「来源 + Methods 含 PUT + Headers `*`」补配，**立即生效不用重启** |
| **同样是"网络错误，上传中断"，但 Console 里写 `Mixed Content`** | 🔴 **服务端签发的预签名 URL 是 `http://`**。`aliyun-sdk-oss` 的 `ClientConfiguration` **默认 `Protocol.HTTP`**，于是直传地址是明文 http，https 页面直接拦掉，请求根本没发出去 —— 所以 OSS 侧只看到失败请求、桶里也不会有对象 | 升级到已修复的 jar（`OssClientConfig` 显式 `setProtocol(Protocol.HTTPS)`）。**只配 CORS 是治不好的**，因为它压根不是跨域问题。回归守卫：`OssClientHttpsTest` |
| 头像能换、文件传不上去 | 头像走**服务端** `PutObject`（同源），文件走**浏览器直传**（跨域 + 独立 URL 协议） | 用"换头像"一步区分：**头像成功 = 服务端 OSS 链路全通**，问题一定在浏览器侧（CORS 或混合内容） |
| 上传大文件能传完但最后一步失败 | CORS 漏了 `Access-Control-Expose-Headers: ETag`，浏览器读不到分片 ETag | §2.5：把 `ETag` 加入「暴露 Headers」 |
| 前端跨域报错 | 只有直连 8081 才会跨域 | 用 §6 的 Nginx 同源代理，不要让前端跨端口访问 |
| **点"下载"后新窗口显示"404 页面不存在"** | 🔴 后端 `download-url` / `preview-url` 返回的是**裸字符串**，而文档与前端都按 `{ url }` 对象处理 → 前端解构出 `undefined`，`<a href="undefined">` 被按相对路径解析成 `https://站点/undefined`，命中前端 `not-found` 路由。**控制台不会报错** | 换用修复后的 jar（两个接口已返回 `FileVo.UrlVo`）。判断方法：`curl .../files/1/download-url` 的 `data` 应该是 `{"url":"..."}`；若是一串裸地址就是旧 jar。契约守卫：`FileUrlContractTest` |
| **管理后台"验证码题库"图片列显示"无图"** | 旧 jar 的题库列表返回裸实体，**没有 `imageUrl` 字段**（前端拿不到可访问的图片地址） | 换用修复后的 jar；前端已加提示"请确认后端已升级到最新版本" |
| **预览时是"一张裂图 + 文件名"** | 🔴 预览当年用的是带 `response-content-disposition` + `response-content-type` **响应头覆盖**的签名 URL；而全站能正常显示图片的路径（头像、验证码、相册、列表缩略图）用的都是**不带覆盖**的普通签名 —— 覆盖是唯一的差异 | 换用修复后的 jar（预览与相册已统一走普通签名）。⚠️ 排查时还发现**下载文件名被双重编码**：调用方先 `URLEncoder.encode` 一次、SDK 再编一次，存下来的文件名成了一串 `%25E7%2585%25A7...`，已改成把原始值交给 SDK 编一次（RFC 5987）。回归守卫：`OssSignServiceTest`。前端现在加载失败会给出"重试 / 下载"的可操作提示，不再只显示裂图 |
| **点某个菜单没反应，Console 报 `Failed to fetch dynamically imported module: .../assets/XXX-<hash>.js`** | 🔴 **发版后浏览器还在用旧主包**：index.html 被缓存了，旧主包引用的页面 chunk（文件名带 hash）在服务器上已不存在 → 404。**只有"以前没打开过"的页面会中招**，所以看起来像"部分菜单坏了" | ① 立刻：`Ctrl+F5` 强刷（或手机上拉刷新）；② 根治：按 §6.1 给 `location = /index.html` 配 `no-store`、`/assets/` 配 `immutable`；③ 自愈：前端 `router.onError` 已内置"自动刷新一次"，配好缓存头后用户基本不会再遇到 |
...
| **换头像报 SQL 错误 `Unknown column 'avatar_updated_at'`** | 🔴 数据库没跑 `migration_v2.2.sql`（v2.2 才加这一列） | 按 §2.3 / §9.4 补跑增量脚本（幂等，可重复执行） |
| 公告接口整片 500 / `Table 'shwork_cloud.announcement' doesn't exist` | 同上，v2.2 才建这张表 | 同上 |
| 换头像返回 40123 | 24 小时冷却未到（**这是功能不是故障**） | 无需处理；前端会显示下次可改时间。要放开就改 `AvatarRules.CHANGE_INTERVAL_HOURS` 重新打包 |
| 公告接口 403 | 公告管理仅超管（`role=9`） | 用超管账号；管理员/教师无权访问 |
| 改公告返回 40095 | 公告处于「已发布」，后端不允许直接改已生效的公告 | 先 `recall` 撤回，改完再 `publish` |
| 公告发布了用户看不到 | `expireTime` 已过、或还处于草稿 | 后台公告列表的 `effective` 列会直接标出「未生效」 |
| **登录 / 上传返回 40105「请先完成人机验证」** | 后台题库已启用，请求没带 `captchaPassToken`（这是功能，不是故障） | 前端应弹验证码窗口（§4.4 ②）；跑冒烟脚本时用 `-CaptchaPassToken <凭证>`，或临时 `CAPTCHA_ENABLED=false` 重启后再跑 |
| 服务器上题库是空的，但登录仍要求验证码 | 题库状态有最长 60 秒缓存；后台增删改题目会立即清除，手工改库不会 | 等 1 分钟，或 `redis-cli DEL cap:enabled` |
| 用户抱怨"每传一个文件都要做验证码" | `CAPTCHA_UPLOAD_PASS_MINUTES` 被设得太小（默认 10 分钟） | 调大该项并重启；注意该项是**分钟** |
| 验证码图片显示不出来 | 题库图片在私有 Bucket，前端拿的是签名地址；若签名失败会返回空 `imageUrl` | 检查 OSS AK/SK 与 `aliyun.oss.endpoint`；后台题库列表的图片列也要能显示 |
| **「社区审核」点右上角「刷新」就报"加载失败：[40000] 参数类型错误: page"** | 🔴 **前端缺陷，不是后端**：模板里写的是 `@click="reload"`，而 Vue 对 `@click="fn"` 的语义是"把事件对象当第一个实参"，于是 `unload(toPage?: number)` 的 `toPage` 收到了一个 `PointerEvent`，请求串变成 `page=[object%20PointerEvent]`，服务端 `Long page` 绑不上 → 40000。**首次进页面能正常加载、只有点刷新才失败**，这一点最容易被误判成后端问题 | 换用修复后的 `dist`（`reload` 已改为只接受 `number`，模板改为 `@click="reload()"`）。**光换 jar 没用**，这是前端代码问题。回归守卫：`npm run check:handlers`（`homework-web/scripts/check-handlers.mjs`，构建前自动跑） |
| 后台侧栏「社区审核」的红点不亮 / 不更新 | 红点由 `GET /admin/posts/pending-count` 驱动，20 秒轮询一次；切回标签页或窗口时会立即补一次 | ① 确认这个接口返回的是 `data: <数字>`；② 若教师账号看不到红点，检查该账号 role 是否为 1/2/9（学生会被 403 挡掉，红点自然不亮）；③ 前端已把"待审数"收敛到 `stores/postReview.ts` 一处，两个页面显示不一致时先看那里 |
